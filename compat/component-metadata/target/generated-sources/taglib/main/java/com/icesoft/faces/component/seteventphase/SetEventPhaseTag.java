...
public class SetEventPhaseTag extends UIComponentTag {
    ...
    protected void setProperties(UIComponent _component) {
        try{
            ...
            if (events != null) {
                if (isValueReference(events)) {
                    ValueBinding _vb = getFacesContext().getApplication().createValueBinding(ELPool.get(events));
                    _component.setValueBinding("events", _vb);
                } else {
                    _component.getAttributes().put("events", events);
                }
            }
            if (phase != null) {
                if (isValueReference(phase)) {
                    ValueBinding _vb = getFacesContext().getApplication().createValueBinding(ELPool.get(phase));
                    _component.setValueBinding("phase", _vb);
                } else {
                    _component.getAttributes().put("phase", phase);
                }
            }
