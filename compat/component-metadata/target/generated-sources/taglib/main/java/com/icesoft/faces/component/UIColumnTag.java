...
public class UIColumnTag extends UIComponentTag {
    ...
    protected void setProperties(UIComponent _component) {
        try{
            ...
            if (colspan != null) {
                if (isValueReference(colspan)) {
                    ValueBinding _vb = getFacesContext().getApplication().createValueBinding(ELPool.get(colspan));
                    _component.setValueBinding("colspan", _vb);
                } else {
                    _component.getAttributes().put("colspan", colspan);
                }
            }
            if (groupOn != null) {
                if (isValueReference(groupOn)) {
                    ValueBinding _vb = getFacesContext().getApplication().createValueBinding(ELPool.get(groupOn));
                    _component.setValueBinding("groupOn", _vb);
                } else {
                    _component.getAttributes().put("groupOn", groupOn);
                }
            }
            ...
            if (rowspan != null) {
                if (isValueReference(rowspan)) {
                    ValueBinding _vb = getFacesContext().getApplication().createValueBinding(ELPool.get(rowspan));
                    _component.setValueBinding("rowspan", _vb);
                } else {
                    _component.getAttributes().put("rowspan", rowspan);
                }
            }
            if (style != null) {
