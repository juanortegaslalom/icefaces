...
public class PanelPositionedTag extends UIComponentTag {
    ...
    public String getComponentType() {
        return "com.icesoft.faces.dragdrop.PanelPositioned";
    }
    ...
    public String getRendererType() {
        return "com.icesoft.faces.dragdrop.PanelPositionedRenderer";
    }
    ...
    protected void setProperties(UIComponent _component) {
        try{
            ...
            if (beforeChangedListener != null) {
                if (isValueReference(beforeChangedListener)) {
                    Class[] panelPositionedArgs = new Class[]{PanelPositionedEvent.class};
                    MethodBinding _mb = getFacesContext().getApplication().createMethodBinding(ELPool.get(beforeChangedListener), panelPositionedArgs);
                    _component.getAttributes().put("beforeChangedListener", _mb);
                } else {
                    throw new IllegalArgumentException(beforeChangedListener);
                }
            }
            if (constraint != null) {
                if (isValueReference(constraint)) {
                    ValueBinding _vb = getFacesContext().getApplication().createValueBinding(ELPool.get(constraint));
                    _component.setValueBinding("constraint", _vb);
                } else {
                    _component.getAttributes().put("constraint", constraint);
                }
            }
            if (disabled != null) {
            ...
            if (handle != null) {
                if (isValueReference(handle)) {
                    ValueBinding _vb = getFacesContext().getApplication().createValueBinding(ELPool.get(handle));
                    _component.setValueBinding("handle", _vb);
                } else {
                    _component.getAttributes().put("handle", handle);
                }
            }
            if (hoverclass != null) {
            ...
            if (listener != null) {
                if (isValueReference(listener)) {
                    Class[] panelPositionedArgs = new Class[]{PanelPositionedEvent.class};
                    MethodBinding _mb = getFacesContext().getApplication().createMethodBinding(ELPool.get(listener), panelPositionedArgs);
                    _component.getAttributes().put("listener", _mb);
                } else {
            ...
            if (overlap != null) {
                if (isValueReference(overlap)) {
                    ValueBinding _vb = getFacesContext().getApplication().createValueBinding(ELPool.get(overlap));
                    _component.setValueBinding("overlap", _vb);
                } else {
                    _component.getAttributes().put("overlap", overlap);
                }
            }
    ...
    private String beforeChangedListener = null;
    public void setBeforeChangedListener(String beforeChangedListener) {
