        ...
        try{
            ...
            if (redisplay != null) {
                if (isValueReference(redisplay)) {
                    ValueBinding _vb = getFacesContext().getApplication().createValueBinding(ELPool.get(redisplay));
                    _component.setValueBinding("redisplay", _vb);
                } else {
                    _component.getAttributes().put("redisplay", Boolean.valueOf(redisplay));
                }
            }
