   ...
   $.jqplot.ColorGenerator = function(colors) {
    ...
    function handleMove(ev, gridpos, datapos, neighbor, plot) {
        if (neighbor) {
            ...
            var evt1 = ice.ace.jq.Event('jqplotDataMouseOver');
            evt1.pageX = ev.pageX;
            evt1.pageY = ev.pageY;
            plot.target.trigger(evt1, ins);
            if (plot.series[ins[0]].highlightMouseOver && !(ins[0] == plot.plugins.lineRenderer.highlightedSeriesIndex)) {
                var evt = ice.ace.jq.Event('jqplotDataHighlight');
		evt.which = ev.which;
                evt.pageX = ev.pageX;
                evt.pageY = ev.pageY;
    ...
    function handleClick(ev, gridpos, datapos, neighbor, plot) {
        if (neighbor) {
            var ins = [neighbor.seriesIndex, neighbor.pointIndex, neighbor.data];
            var evt = ice.ace.jq.Event('jqplotDataClick');
	    evt.which = ev.which;
            evt.pageX = ev.pageX;
            evt.pageY = ev.pageY;
            plot.target.trigger(evt, ins);
    ...
    function handleRightClick(ev, gridpos, datapos, neighbor, plot) {
        if (neighbor) {
            ...
            var evt = ice.ace.jq.Event('jqplotDataRightClick');
	    evt.which = ev.which;
            evt.pageX = ev.pageX;
            evt.pageY = ev.pageY;
            plot.target.trigger(evt, ins);
...
var backCompat = ice.ace.jq.uiBackCompat !== false;
    ...
    ice.ace.jq.extend( ice.ace.jq.jqplot.effects, {
        version: "1.9pre",
        ...
        createWrapper: function( element ) {
            ...
            if ( element[ 0 ] === active || ice.ace.jq.contains( element[ 0 ], active ) ) {
                ice.ace.jq( active ).focus();
            }
            ...
            if ( element.css( "position" ) === "static" ) {
                ...
                ice.ace.jq.extend( props, {
                    position: element.css( "position" ),
                    zIndex: element.css( "z-index" )
                });
                ice.ace.jq.each([ "top", "left", "bottom", "right" ], function(i, pos) {
    ...
    function _normalizeArguments( effect, options, speed, callback ) {
        ...
        if ( ice.ace.jq.isPlainObject( effect ) ) {
            return effect;
        }
        ...
        if ( ice.ace.jq.isFunction( options ) ) {
            callback = options;
            speed = null;
            options = {};
        }
        ...
        // catch (effect, speed, ?)
        if ( ice.ace.jq.type( options ) === "number" || ice.ace.jq.fx.speeds[ options ]) {
            callback = speed;
            speed = options;
            options = {};
        }
        ...
        if ( ice.ace.jq.isFunction( speed ) ) {
            callback = speed;
            speed = null;
        }
        ...
        if ( options ) {
            ice.ace.jq.extend( effect, options );
        }

        speed = speed || options.duration;
        effect.duration = ice.ace.jq.fx.off ? 0 : typeof speed === "number"
            ? speed : speed in ice.ace.jq.fx.speeds ? ice.ace.jq.fx.speeds[ speed ] : ice.ace.jq.fx.speeds._default;
    ...
    function standardSpeed( speed ) {
        ...
        if ( !speed || typeof speed === "number" || ice.ace.jq.fx.speeds[ speed ] ) {
            return true;
        }

        // invalid strings - treat as "normal" speed
        if ( typeof speed === "string" && !ice.ace.jq.jqplot.effects.effect[ speed ] ) {
            // TODO: remove in 2.0 (#7115)
            if ( backCompat && ice.ace.jq.jqplot.effects[ speed ] ) {
                return false;
            }
            return true;
        }
    ...
    ice.ace.jq.fn.extend({
        jqplotEffect: function( effect, options, speed, callback ) {
            var args = _normalizeArguments.apply( this, arguments ),
                mode = args.mode,
                queue = args.queue,
                effectMethod = ice.ace.jq.jqplot.effects.effect[ args.effect ],

                // DEPRECATED: remove in 2.0 (#7115)
                oldEffectMethod = !effectMethod && backCompat && ice.ace.jq.jqplot.effects[ args.effect ];

            if ( ice.ace.jq.fx.off || !( effectMethod || oldEffectMethod ) ) {
            ...
            function run( next ) {
                var elem = ice.ace.jq( this ),
                    complete = args.complete,
                    mode = args.mode;

                function done() {
                    if ( ice.ace.jq.isFunction( complete ) ) {
                        complete.call( elem[0] );
                    }
                    if ( ice.ace.jq.isFunction( next ) ) {
                        next();
                    }
                }
    ...
    ice.ace.jq.jqplot.effects.effect.blind = function( o, done ) {
        // Create element
        var el = ice.ace.jq( this ),
            props = [ "position", "top", "bottom", "left", "right", "height", "width" ],
            mode = ice.ace.jq.jqplot.effects.setMode( el, o.mode || "hide" ),
        ...
        if ( el.parent().is( ".ui-effects-wrapper" ) ) {
            ice.ace.jq.jqplot.effects.save( el.parent(), props );
        } else {
            ice.ace.jq.jqplot.effects.save( el, props );
        }
        el.show();
        top = parseInt(el.css('top'), 10);
        wrapper = ice.ace.jq.jqplot.effects.createWrapper( el ).css({
            overflow: "hidden"
        });
        ...
        wrapper.animate( animation, {
            ...
            complete: function() {
                ...
                ice.ace.jq.jqplot.effects.restore( el, props );
                ice.ace.jq.jqplot.effects.removeWrapper( el );
                done();
            }
        });
    ...
    function handleMove(ev, gridpos, datapos, neighbor, plot) {
        ...
        if (hl.show) {
            if (neighbor == null && hl.isHighlighting) {
                var evt = ice.ace.jq.Event('jqplotHighlighterUnhighlight');
                ...
                var evt = ice.ace.jq.Event('jqplotHighlighterHighlight');
                evt.which = ev.which;
                evt.pageX = ev.pageX;
                evt.pageY = ev.pageY;
