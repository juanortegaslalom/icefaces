...
import org.icefaces.ace.component.buttongroup.IButtonGroup;
/*
...
public abstract class ButtonGroupBase extends javax.faces.component.UIComponentBase implements IButtonGroup{
	...
	public static final String COMPONENT_TYPE = "org.icefaces.ace.component.ButtonGroup";
	public static final String RENDERER_TYPE = "org.icefaces.ace.component.ButtonGroupRenderer";
	...
	public String getFamily() {
		return "org.icefaces.ace.component.ButtonGroup";
	}
	 ...
	 * <p>Contents: Header text for the button group.</p>
	 */
	...
	/**
	 * <p>Set the value of the <code>mutuallyExclusive</code> property.</p>
	 * <p>Contents: If true, only one checkboxButton in the group can be selected at one time, selecting a new checkboxButton deselects the previously selected one.</p>
	 */
	public void setMutuallyExclusive(boolean mutuallyExclusive) {
		PropertyKeys propToUse = PropertyKeys.mutuallyExclusive;
		addToSetOfUsedProperties(propToUse);
		ValueExpression ve = getValueExpression(PropertyKeys.mutuallyExclusive.toString() );
		if (ve != null) {
			// map of style values per clientId
			ve.setValue(getFacesContext().getELContext(), mutuallyExclusive );
		} else { 
			StateHelper sh = getStateHelper(); 
			if (isDisconnected(this))  {
				String defaultKey = PropertyKeys.mutuallyExclusive.toString() + "_defaultValues";
				Map clientDefaults = (Map) sh.get(defaultKey);
				if (clientDefaults == null) { 
					clientDefaults = new HashMap(); 
					clientDefaults.put("defValue",mutuallyExclusive);
					sh.put(defaultKey, clientDefaults); 
				} 
			} else {
				String clientId = getClientId();
				String valuesKey = PropertyKeys.mutuallyExclusive.toString() + "_rowValues"; 
				Map clientValues = (Map) sh.get(valuesKey); 
				if (clientValues == null) {
					clientValues = new HashMap(); 
				}
				clientValues.put(clientId, mutuallyExclusive);
				//Always re-add the delta values to the map. JSF merges the values into the main map
				//and values are not state saved unless they're in the delta map. 
				sh.put(valuesKey, clientValues);
			}
	...
	/**
	 * <p>Return the value of the <code>mutuallyExclusive</code> property.</p>
	 * <p>Contents: If true, only one checkboxButton in the group can be selected at one time, selecting a new checkboxButton deselects the previously selected one. Default = 'false'.</p>
	 */
